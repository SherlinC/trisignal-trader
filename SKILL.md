...
1. 存在唯一最优标的
2. 最优标的评分 **≥ 7.5 分**，且显著领先其他标的（差距 ≥ 1.5 分）
...
> **加仓说明**：同一标的同方向加仓时，仓位合并计算。新 OCO 止损止盈单仅覆盖本次新增的 sz，原有止损止盈单保持不变。加仓的 sz 计算方式与首仓相同（基于账户净值 8% 风险额）。加仓条件：持仓标的评分仍 ≥ 7.5 且浮盈 > 0（不在亏损时加仓）。
