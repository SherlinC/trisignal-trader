...
BTC 是加密市场风向标。当 BTC MACD 信号与候选标的方向冲突时，全市场该方向风险上升：
...
**做空方向降级**：
- **触发条件**：BTC 的 DIF > DEA 且 Hist > 0（MACD 金叉确认，大盘偏多）
- **影响**：所有标的（含 BTC 自身）的做空方向评分额外 **-1 分**

**记录要求**：在 snapshot 的 `anomalies` 中标注 `"BTC大盘过滤：MACD死叉/金叉，全标的做多/做空-1"`
...
完全反排（MA5<MA10<MA20<MA60）形成较慢，以下"准空头"结构可提前识别做空机会：

**准空头结构（D1 可给 6-7 分）**：
- MA5 < MA10 < MA20，且 MA20 开始走平或拐头向下（即使 MA60 还在下方）
- 收盘价跌破 MA20 且连续 2 根未收回
- MA5/MA10 死叉后间距在扩大（发散中）

**空头加速确认（D1 可给 8-9 分）**：
- MA5 < MA10 < MA20 < MA60 完全反排
- 或 MA5 < MA10 < MA20，且 MA60 已走平（即将被穿越）

**做空 MACD 信号强度判断**：
- DIF < DEA 且 Hist 连续 3 根递减（加速下行）→ D2 = 8-9
- DIF < DEA 且 Hist < 0 但绝对值缩小（动能减弱）→ D2 = 5-6
- DIF 和 DEA 都在零轴下方 → 空头趋势已确立，额外 +0.5

**做空 OI 配合**：
- 价格下跌 + OI 增加 → 新空头资金入场，D4 = 8-9
- 价格下跌 + OI 减少 → 多头止损出场但无新空头，D4 = 5-6

**做空 funding 信号**：
- funding 为正且偏高（> 0.03%）→ 多头拥挤，做空有利，D5 加分
- funding 为负且偏高（< -0.03%）→ 空头已拥挤，做空风险上升，D5 降分
...
评分完成后，必须对每个标的**分别评估做多和做空两个方向**，选出全局最优的（标的+方向）组合：

**Step A — 多头方向评估**：
对每个标的，按 7 维度评分，评估做多信号强度。多头加分条件：MA 顺排发散、DIF>DEA、Hist>0、高低点抬高等。

**Step B — 空头方向评估**：
对每个标的，按 7 维度评分，评估做空信号强度。空头加分条件：MA 反排发散（MA5<MA10<MA20<MA60）、DIF<DEA、Hist<0、高低点降低等。

> 注意：同一标的的多头评分和空头评分通常不同。例如 BTC 均线多头顺排时，多头 D1=7 但空头 D1=2。

**Step C — 选出全局最优**：
从所有（标的×方向）组合中选出评分最高的一个作为候选。

**方向确认规则**：
- **做多**：MA5>MA10>MA20>MA60 顺排 + DIF>DEA + Hist>0
- **做空**：MA5<MA10<MA20<MA60 反排 + DIF<DEA + Hist<0
- **方向不明**：均线方向与 MACD 方向矛盾 → 该方向评分上限 5 分

> 均线方向与 MACD 方向必须一致才允许开仓。任何矛盾信号 → 该方向不可开仓。

**输出格式**：评分结果中标注方向，如 `ETH：7.9/10 做多 — 四线发散+MACD金叉` 或 `BTC：8.0/10 做空 — 反排+MACD死叉加速`

**BTC 大盘过滤应用**：
- 做多候选：若 BTC MACD 死叉 → 全标的做多评分 -1
- 做空候选：若 BTC MACD 金叉 → 全标的做空评分 -1
