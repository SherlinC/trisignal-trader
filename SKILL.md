...
| 模式 | 说明 |
|------|------|
| `paper`（默认） | 完整执行分析、评分、决策、仓位计算、理论下单止损、记录输出；主要目标是记录与验证 |
| `shadow` | 完整运行策略逻辑，生成完整参数与记录，不提交影响真实资产的订单 |
| `live` | 正式实盘，仅在 paper/shadow 长期稳定后使用，任一关键校验失败则放弃下单 |

**模式切换原则**：默认 `paper`，未经明确确认不切换到 `live`，daily review 不得自动切换模式。
...
**必须记录**：账户净值、风险额、开仓价、止损价、单位风险、计算出的 sz
